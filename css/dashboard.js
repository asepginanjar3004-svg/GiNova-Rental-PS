...
// Export ke window agar bisa diakses payment.js
...
async function syncPendingFnBOrders(showToast = false) {
  ...
  if (showToast) showToast('Sedang menyinkronkan data...', 'info');
  ...
  if (remaining.length > 0) {
    ...
    if (showToast) showToast(`${remaining.length} data gagal sinkron.`, 'warning');
  } else {
...
function confirmBooking(consoleId) {
  // Placeholder: Implement booking confirmation logic
  alert('Fitur booking belum diimplementasi. Gunakan Mulai Main untuk sekarang.');
...
function cancelBooking(consoleId) {
  ...
  if (confirm('Batalkan booking untuk unit ini?')) {
    ...
    if (console) {
      ...
      db.update(DB_KEYS.CONSOLES, console);
  ...
  try {
    ...
    if (window.location.protocol.startsWith('http')) {
      try {
        ...
        const modal = bootstrap.Modal.getInstance(document.getElementById('addFnBModal'));
        if (modal) modal.hide();
        refreshDashboard();
        return;
      } catch (ajaxError) {
...
function voidFnBItem(itemId) {
  ...
  const prod = db.getById(DB_KEYS.PRODUCTS, item.product_id);
  if (prod) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Sinkronisasi otomatis saat internet/koneksi kembali tersambung
  ...
  // Handler untuk tombol "Sinkron Sekarang" di UI Alert
  const btnManualSync = document.getElementById('btnSyncNow');
  if (btnManualSync) {
    btnManualSync.addEventListener('click', () => syncPendingFnBOrders(true));
  }
